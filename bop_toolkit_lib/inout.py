  ...
  for im_id in scene_camera.keys():
    if 'cam_K' in scene_camera[im_id].keys():
      scene_camera[im_id]['cam_K'] = \
        np.array(scene_camera[im_id]['cam_K'], np.floating).reshape((3, 3))
    if 'cam_R_w2c' in scene_camera[im_id].keys():
      scene_camera[im_id]['cam_R_w2c'] = \
        np.array(scene_camera[im_id]['cam_R_w2c'], np.floating).reshape((3, 3))
    if 'cam_t_w2c' in scene_camera[im_id].keys():
      scene_camera[im_id]['cam_t_w2c'] = \
        np.array(scene_camera[im_id]['cam_t_w2c'], np.floating).reshape((3, 1))
  return scene_camera
...
def load_scene_gt(path):
  ...
  for im_id, im_gt in scene_gt.items():
    for gt in im_gt:
      if 'cam_R_m2c' in gt.keys():
        gt['cam_R_m2c'] = np.array(gt['cam_R_m2c'], np.floating).reshape((3, 3))
      if 'cam_t_m2c' in gt.keys():
        gt['cam_t_m2c'] = np.array(gt['cam_t_m2c'], np.floating).reshape((3, 1))
  return scene_gt
      ...
      for line in f:
        ...
        if line_id == 1 and header in line:
          ...
          result = {
            ...
            'score': float(elems[3]),
            'R': np.array(
              list(map(float, elems[4].split())), np.floating).reshape((3, 3)),
            't': np.array(
              list(map(float, elems[5].split())), np.floating).reshape((3, 1)),
            'time': float(elems[6])
          }
